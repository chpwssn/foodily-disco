...
TRACKER_ID = 'foodilydisco'
TRACKER_HOST = '10.0.0.35:9080'
...
class CustomProcessArgs(object):
    def realize(self, item):
        ...
        if item_type == 'tag':
            ...
            size, page = item_value.split(',', 1)
            return ['python', 'tagdisco.py', "-p "+str(page), "-s "+str(size),
                    "-F%(item_dir)s/%(warc_file_base)s.txt" % item]
        else:
...
SCRIPT_SHA1 = get_hash(os.path.join(CWD, 'tagdisco.py'))
